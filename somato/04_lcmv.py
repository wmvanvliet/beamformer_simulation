...
inv = mne.beamformer.make_lcmv(epochs.info, fwd, data_cov, reg=0.05, noise_cov=noise_cov)
stc = mne.beamformer.apply_lcmv(epochs.average(), inv)
stc.subject = subject_id
stc.save(fname.stc_lcmv)
peak_time = stc.get_peak()[1]
abs(stc.copy().crop(peak_time, peak_time)).save_as_volume(fname.nii_lcmv, src=fwd['src'])

fig = stc.plot(initial_time=peak_time, subject=subject_id, subjects_dir=fname.subjects_dir, src=fwd['src'],
